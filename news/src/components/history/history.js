...
function History() {
  ...
  return (
    <div
      style={{ margin: 0 + " " + 100 + "px", minHeight:'230px' }}
      className={categoryStyles["topStory-15nd"]}
    >
      <h1 className={categoryStyles.title}>CÁC BÀI ĐÃ XEM GẦN ĐÂY</h1>
      {reversedTop15Story?.length > 0 ? (
        reversedTop15Story.map((item, index) => (
          <div
            key={index}
            className={` ${categoryStyles.horizontalPost} ${
              categoryStyles["version-news"]
            } ${"mb-20"}  `}
          >
            <div
              className={` ${categoryStyles["horizontalPost__avt"]} ${categoryStyles["avt-240"]} `}
            >
              <Link
                to={"/detail/" + item.id}
                title={item.title}
              ...
              >
                <picture>
                  ...
                  {item.img ? (
                    ...
                    <FaVideo
                      style={{ fontSize: "70px", margin: "30px 85px" }}
                    />
                  )}
                </picture>
              </Link>
            ...
            <div className={categoryStyles["horizontalPost__main"]}>
              <h3
                className={` ${categoryStyles["horizontalPost__main-title"]} ${categoryStyles["vnn-title"]} ${categoryStyles["title-bold"]} `}
                data-id="2291894"
                ispr="False"
              >
                <Link
              ...
              <div
                className={categoryStyles["horizontalPost__main-desc"]}
                data-limit=""
              >
        ...
        <div style={{textAlign:'center', marginTop:'40px'}}>
          <p style={{fontSize:'30px', fontWeight:'bold'}}>Không có dữ liệu</p>
        </div>
      )}
    </div>
  );
